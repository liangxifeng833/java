 ...
 *
 * InvokeAll方法处理一个任务的容器（collection），并返回一个Future的容器。
...
public class InvokeAllThread {
    ...
    public void getRankedTravelQuotes() throws InterruptedException {
        ...
        //模拟10个计算报价的任务
        for (int i=1; i <= 10; i++) {
            tasks.add(new QuoteTask(100,i));
        }

        /**
         * 使用invokeAll方法批量提交限时任务任务 预期5s所有任务都执行完,没有执行完的任务会自动取消
         */
        List<Future<BigDecimal>> futures = mExecutor.invokeAll(tasks, 5, TimeUnit.SECONDS);
        // 报价合计集合
        ...
        for (BigDecimal bigDecimal : totalPriceList) {
            System.out.println("计算结果:"+bigDecimal);
        }
        mExecutor.shutdown();
    }
    ...
    /**
     * 输出:
     耗时：2s,单价是：100.0,人数是：2，总额是：200.00
     耗时：4s,单价是：100.0,人数是：1，总额是：100.00
     任务超时，取消计算,单价是100.0，人数是：3
     任务超时，取消计算,单价是100.0，人数是：4
     任务超时，取消计算,单价是100.0，人数是：5
     任务超时，取消计算,单价是100.0，人数是：6
     任务超时，取消计算,单价是100.0，人数是：7
     任务超时，取消计算,单价是100.0，人数是：8
     任务超时，取消计算,单价是100.0，人数是：9
     任务超时，取消计算,单价是100.0，人数是：10
     计算结果:100.00
     计算结果:200.00
     计算结果:0
     计算结果:0
