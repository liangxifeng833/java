...
$(function () {
    ...
    $('.user-right .subscribe button').on('click', function () {
        ...
        layer.confirm('确认要取消预约吗', {
            ...
            $.ajax({
                url: '/user/house/subscribe?houseId=' + selected,
                type: 'DELETE',
                success: function (data) {
                    if (data.code === 200) {
    ...
    layui.use('flow', function () {
        ...
        flow.load({
            ...
            scrollElem: '.wait-record .content',
                ...
                $.get('/user/house/subscribe/list?status=1' + '&start=' + start + '&size=3',
                    function (res) {
    ...
    layui.use('flow', function () {
        ...
        flow.load({
            ...
            scrollElem: '.subscribe .content',
                ...
                $.get('/user/house/subscribe/list?status=2' + '&start=' + start + '&size=3',
                    function (res) {
                        layui.each(res.data, function (index, tuple) {
                            var house = tuple.first,
    ...
    layui.use('flow', function () {
        ...
        flow.load({
            ...
            scrollElem: '.finish-record .content',
                ...
                $.get('/user/house/subscribe/list?status=3' + '&start=' + start + '&size=3',
                    function (res) {
                        layui.each(res.data, function (index, tuple) {
                            var house = tuple.first,
                                subscribe = tuple.second;
